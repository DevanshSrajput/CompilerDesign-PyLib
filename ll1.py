...
def build_ll1_table(grammar: dict, start: str = None) -> dict:
    ...
    Returns:
        {
          'table': dict mapping (NonTerminal, Terminal) -> production string,
...
def ll1_parse(grammar: dict, tokens: list, start: str = None) -> dict:
    ...
    tokens: list of terminal strings (do NOT include '$', it's added automatically)

    Returns:
        {
          'accepted': bool,
          'steps': list of parse step dicts,
          'error': str or None
        }
    """
    if start is None:
